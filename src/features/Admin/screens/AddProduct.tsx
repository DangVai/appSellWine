...
const COLUMN_WIDTH = (width - 48) / 2; // 48 là tổng padding trái phải và khoảng cách giữa 2 cột
...
const Sanpham3Sqlite = () => {
    ...
    const handlePickImage = async () => {
        ...
        let result = await ImagePicker.launchImageLibraryAsync({
            ...
            allowsEditing: true, // Cho phép crop ảnh vuông cho đẹp
            aspect: [1, 1],
            quality: 0.8,
        });
    ...
    const getImageSource = (img?: string | null) => {
        const finalImg = img ?? 'hinh1.jpg';
        if (finalImg.startsWith('file://')) return { uri: finalImg };
        switch (finalImg) {
            case 'hinh1.jpg': return require('../../../assets/images/products/chup-anh-quang-cao-chai-ruou-vang-wine-bottle-photography_0001.jpg');
            default: return require('../../../assets/images/products/chup-anh-quang-cao-chai-ruou-vang-wine-bottle-photography_0001.jpg');
        }
    };
    ...
    const displayedProducts = products.filter(p =>
        // treat both null and undefined as "Tất cả" (show everything)
        filterCategoryId == null ? true : p.categoryId === filterCategoryId
    );
    // --- RENDER ITEM GIAO DIỆN LƯỚI (GRID) ---
            ...
            <View style={styles.gridInfo}>
                ...
                <View style={styles.gridActions}>
                    <TouchableOpacity onPress={() => handleEdit(item.id as number)} style={styles.actionBtn}>
                        <Text style={[styles.actionText, { color: '#007bff' }]}>Sửa</Text>
                    </TouchableOpacity>
        ...
        <View style={{ flex: 1, backgroundColor: '#f2f2f2' }}>
            ...
            <View style={{ flex: 1 }}>
                            ...
                            <View style={styles.rowInput}>
                                <View style={{ width: 110 }}>
                                    <RNPickerSelect
                                        ...
                                        placeholder={{ label: "Tất cả", value: null }} // Đây chính là nút "Xem hết"
                                        style={{
                                            inputAndroid: {
                                                fontSize: 13, color: '#333', paddingRight: 30
                                            },
                                            placeholder: { color: '#007bff', fontWeight: 'bold' }
                                        }}
                                        useNativeAndroidPickerStyle={false} // Tắt style mặc định để dễ chỉnh
                                    />
                                    {/* Mũi tên nhỏ để người dùng biết là menu xổ xuống */}
                ...
                <View style={styles.listSection}>
                    ...
                    <FlatList
                        key={'grid-2-columns'} // Key cố định để force render dạng lưới
                        data={displayedProducts}
                        numColumns={2} // QUAN TRỌNG: Hiển thị 2 cột
                        keyExtractor={(item) => (item.id as number).toString()}
                        renderItem={renderItem}
                        contentContainerStyle={styles.listContent}
                        columnWrapperStyle={styles.columnWrapper} // Style cho hàng ngang
...
const styles = StyleSheet.create({
    ...
    columnWrapper: { justifyContent: 'space-between' }, // Căn đều 2 bên

    // GRID ITEM STYLES (Giao diện thẻ bài)
    gridCard: {
        width: COLUMN_WIDTH,
        backgroundColor: '#fff',
        borderRadius: 12,
    ...
    gridInfo: { padding: 8 },
    gridName: { fontSize: 13, fontWeight: '600', color: '#333', height: 36, marginBottom: 4 }, // Giới hạn chiều cao tên
